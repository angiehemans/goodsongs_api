class EventCommentsController < ApplicationController
  ...
  def index
    ...
    comments = @event.event_comments.includes(:user).chronological
  ...
  def create
    ...
    @comment = @event.event_comments.build(comment_params.merge(user: current_user))

    if @comment.save
      # Notify the event creator
      Notification.notify_event_comment(event: @event, commenter: current_user, comment: @comment)
