class Auth::ThreadsController < ApplicationController
  ...
  def authorize
    ...
    url = ThreadsOauthService.new.authorize_url(state: state)
  ...
  def callback
    ...
    service = ThreadsOauthService.new
    ...
    account = user.connected_accounts.find_or_initialize_by(platform: "threads")
    account.update!(
    ...
    redirect_to "#{frontend_url}/settings/connections?status=success&platform=threads", allow_other_host: true
  rescue ActiveSupport::MessageVerifier::InvalidSignature
    frontend_url = ENV.fetch("FRONTEND_URL", "https://goodsongs.app")
    redirect_to "#{frontend_url}/settings/connections?status=error&platform=threads", allow_other_host: true
  rescue StandardError => e
    Rails.logger.error("Threads OAuth error: #{e.message}")
    frontend_url = ENV.fetch("FRONTEND_URL", "https://goodsongs.app")
    redirect_to "#{frontend_url}/settings/connections?status=error&platform=threads", allow_other_host: true
  end
end
